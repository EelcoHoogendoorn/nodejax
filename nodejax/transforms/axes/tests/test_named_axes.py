...
def test_unbatched_binds_its_named_axis_during_construction() -> None:
    def init(input):
        return input
    ...
    normalizer = unbatched(nn.BatchNorm(momentum=0.1)).with_input(input)
    normalizer = normalizer.parameterize().initialize()
    pipe = stack(layer, n=2) >> nn.Linear(2)
    model = unbatched(pipe).with_input(input).parameterize(
        rng=jax.random.PRNGKey(0)
    ).initialize(input=input)

    stacked_state = model.state.stack_primed_batch_norm
    successor, output = model(input)

    assert normalizer.state.mean.shape == (3,)
    assert normalizer.state.var.shape == (3,)
    assert model.param.linear.w.shape == (3, 2)
    assert stacked_state.primed.shape == (2, 3)
    assert stacked_state.batch_norm.mean.shape == (2, 3)
    assert stacked_state.batch_norm.var.shape == (2, 3)
    assert successor.state.stack_primed_batch_norm.primed.shape == (2, 3)
