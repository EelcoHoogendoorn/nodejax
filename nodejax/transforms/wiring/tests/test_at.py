...
def test_at_routes_field():
    ...
    out = node.apply(x=jnp.asarray(3.0), y=jnp.asarray(7.0))
...
def test_at_cyclic_state_threads():
    node = at(Integrator(), 'x').parameterize()
    state = node.init()
    state, out = node.apply(state, x=jnp.asarray(2.0), y=jnp.asarray(9.0))
    state, out2 = node.apply(state, x=jnp.asarray(2.0), y=jnp.asarray(9.0))
