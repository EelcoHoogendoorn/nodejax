...
def test_parallel_routes_strands():
    block = parallel(x=gain_def(), y=ident).parameterize(x=Struct(scale=jnp.asarray(2.0)))
...
def test_parallel_cyclic_state_threads():
    block = parallel(x=integrator_def(), y=ident).parameterize(x=Struct(gain=jnp.asarray(1.0)))
    state = block.init()
...
def test_parallel_is_strict():
    ...
    block = parallel(x=gain_def()).parameterize(x=Struct(scale=jnp.asarray(2.0)))
...
def test_parallel_init_splits_rng_to_stochastic_members():
    ...
    block = parallel(x=noisy_def(), y=integrator_def()).parameterize(
