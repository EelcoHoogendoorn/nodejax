...
def test_residual():
    r = residual(gain_def()).parameterize(scale=jnp.array(2.0))
    assert jnp.allclose(r.apply(3.0), 9.0)            # x + 2x

    bound = gain_def().parameterize(scale=jnp.array(0.5))
...
def test_residual_cyclic():
    ...
    r = residual(integrator_def()).parameterize(gain=jnp.array(1.0))
    state = r.init()
