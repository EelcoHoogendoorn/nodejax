...
def test_contract_init_primes_from_the_input_channel():
    from nodejax.examples.comparisons.imu.imu_nodejax import Derivative
    d = Derivative(0.1)
    # input is its OWN channel, typed by apply_input_spec — never a bundle field
    s = _init(d.node, (), input=jnp.asarray(5.0))
    assert jnp.allclose(s, 5.0)                # primed from the real value
    assert d.contract.init_requires_input          # the record, stored on the node
    with pytest.raises(TypeError, match='unknown bundle fields'):
        d.init(Struct(input=jnp.asarray(5.0)))
