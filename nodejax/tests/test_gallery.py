...
def test_stack_scan_commute():
    ...
    per_step = scan(stack(Integrator())).parameterize(gain=gains)
    per_layer = stack(scan(Integrator())).parameterize(gain=gains)
...
def test_bptt_system_identification():
    ...
    seq = scan(Leaky())  # PCN -> PN: maps input sequence -> output sequence
...
def test_gradient_pid_tuning():
    ...
    loop = feedback(PD(dt) >> Plant(dt, 1.0, 0.3).ndef)  # reference -> position
...
def test_feedback_mimo():
    ...
    loop = feedback(PD(dt) >> Plant(dt, spring_k, 0.3).ndef)
