...
def StatefulLinearNode(D: int = 4):
    ...
    return node_def(apply, param=param, init=init, name='stateful_linear')
...
def StochasticNode():
    ...
    return node_def(apply, param=param, init=init, name='stochastic_node')
...
def test_batch_scan_and_scan_batch():
    """Sequence processing over batched data: batch(scan) and scan(batch)."""
    D = 4
    T = 6
    B = 5
    ...
    # 1. batch(scan(nd)) - scan input is (T, D), batched input is (B, T, D)
    bsc = batch(scan(StatefulLinear), n=B).with_input(jnp.zeros((B, T, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_bsc = bsc.init()
    assert s_bsc == ()
    ...
    # 2. scan(batch(nd)) - batch input is (B, D), sequence input is (T, B, D)
    scb = scan(batch(StatefulLinear, n=B)).with_input(jnp.zeros((T, B, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_scb = scb.init()
    assert s_scb == ()
...
def test_ensemble_stack_and_ensemble_scan():
    ...
    # 1. ensemble(stack(nd, n=L), n=N_ens)
    ...
    # 2. ensemble(scan(nd), n=N_ens)
    escan = ensemble(scan(StatefulLinear), n=N_ens).with_input(jnp.zeros((T, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_escan = escan.init()
    assert s_escan == ()
...
def test_batch_freeze_and_freeze_batch():
    ...
    B = 5
    StatefulLinear = StatefulLinearNode(D)
    nd = StatefulLinear.with_input(jnp.zeros((D,))).parameterize(rng=jax.random.PRNGKey(0))
    st = nd.init()

    # 1. batch(freeze(nd, st))
    bf = batch(freeze(nd, st), n=B).with_input(jnp.zeros((B, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_bf = bf.init()
    assert s_bf == ()
...
def test_batch_at_and_stack_at():
    ...
    input_batch = Struct(x=jnp.ones((B, D)), y=jnp.ones((B, 2)))
    s_bat2, out_bat = bat.apply(s_bat, input_batch)
    assert out_bat.x.shape == (B, D)
    assert jnp.allclose(out_bat.y, input_batch.y)

    # 2. stack(at(nd, 'x'), n=L)
    sat = stack(at(StatefulLinear, 'x'), n=L).with_input(sample_input).parameterize(rng=jax.random.PRNGKey(0))
    s_sat = sat.init()

    s_sat2, out_sat = sat.apply(s_sat, sample_input)
...
def test_rng_splitting_across_batch_and_ensemble():
    ...
    x_batch = jnp.ones((B, D))
    input_with_rng = Struct(rng=jax.random.PRNGKey(42), x=x_batch)

    # Calling apply with rng attached to input
    s2, out = be_stoch.apply(s, input_with_rng)
...
def test_map_members_rebuild_on_nested_towers():
    ...
    new_l2 = nn.Linear(D)
    rebuilt_tower = map_members(tower, lambda m: new_l2 if m.name == 'l2' else m)
...
def test_batch_ensemble_scan_triple_tower():
    """Batch of ensembles of sequence models: batch(ensemble(scan(nd)))."""
    D = 4
    N_ens = 3
    T = 6
    B = 5
    ...
    StatefulLinear = StatefulLinearNode(D)
    bes = batch(ensemble(scan(StatefulLinear), n=N_ens), n=B).with_input(jnp.zeros((B, T, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_bes = bes.init()
    assert s_bes == ()
...
def test_batch_ensemble_stack_triple_tower():
    """Batch of ensembles of stacked layers: batch(ensemble(stack(nd, n=L)))."""
    D = 4
    N_ens = 3
    L = 4
    B = 5
...
def test_batch_repeat_freeze_triple_tower():
    """Batched sequence processing over a frozen non-cyclic model: batch(repeat(freeze(nd, st)))."""
    D = 4
    T = 6
    B = 5

    StatefulLinear = StatefulLinearNode(D)
    nd = StatefulLinear.parameterize(rng=jax.random.PRNGKey(0))
    st = nd.init()

    brf = batch(repeat(freeze(nd, st), n=T), n=B).with_input(jnp.zeros((B, T, D))).parameterize(rng=jax.random.PRNGKey(0))
    s_brf = brf.init()
    assert s_brf == ()
