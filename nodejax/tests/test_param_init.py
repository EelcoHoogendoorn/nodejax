...
def test_explicit_values_need_no_rng():
    ...
    node = linear_def(2, 3).bind(Struct(weight=jnp.ones((2, 3)), bias=jnp.ones(3)))
    assert jnp.allclose(node.apply(jnp.ones(2)), 3.0)
    with pytest.raises(TypeError, match='missing required'):
        linear_def(2, 3).parameterize(weight=jnp.ones((2, 3)), bias=jnp.ones(3))


def test_overrides_mix_with_draws():
    node = linear_def(2, 3).parameterize(rng=jax.random.PRNGKey(0), bias=jnp.ones(3))
    assert jnp.allclose(node.param.bias, 1.0)
    assert node.param.weight.shape == (2, 3)


def test_pipe_splits_param_rng_per_member():
    pipe = linear_def(2, 2) >> linear_def(2, 2)
