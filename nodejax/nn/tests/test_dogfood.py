...
Yet, we never see a linear layer written as
    linear = ensemble(nn.Projection(), n)
...
projections, the unit defined once and the output axis added by a
...
from nodejax import nn, trained, ensemble, batch, train_step
...
def test_linear_is_an_ensemble_of_projections():
    """ensemble(Projection) is a linear layer: params one row per unit,
    input broadcast, outputs stacked — and numerically the matrix. The
    SAME def sizes itself to whatever width it is bound at."""
    linear = ensemble(nn.Projection(), n=3)
...
def test_the_ensemble_layer_trains_like_any_layer():
    ...
    model = batch(ensemble(nn.Projection(), n=3)).with_input(jnp.zeros((16, 4))).parameterize(
...
def test_an_mlp_is_units_all_the_way_down():
    ...
    is ensemble(Projection), the readout is ONE Projection, the mlp is
    their pipe — no layer anywhere written as a matrix. Resolved and
    parameterized per-sample; batch() then shares the params."""
    mlp = ensemble(nn.Projection(), n=16) >> nn.gelu >> nn.Projection()
