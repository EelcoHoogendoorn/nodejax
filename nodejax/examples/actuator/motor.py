...
electrical_def is THE motor: params, the model equations as methods,
and the electrical dynamics. The same def serves every role — the plant
in the assembled stack, half of bench_motor_def (its composition with
...
@ambient
def electrical_def(dt, substeps=4):
...
def bench_motor_def(dt):
    ...
    members = dict(electrical=electrical_def(dt), mechanical=mechanical_def(dt))
