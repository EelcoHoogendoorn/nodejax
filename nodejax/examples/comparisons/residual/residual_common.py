...
Six behaviours are the parity contract and are asserted. ``nests`` is
measured and printed, for the reason given in the lifted-stack harness: it is
the criterion the columns disagree on, and asserting it would only make the
failing column stop before it reported.
"""
...
class Evidence:
    ...
    aux_escaped: bool
    #: The wrapper accepts its own product, around BOTH a deterministic and
    #: a stochastic member. Both, because the asymmetry is the finding: a
    #: wrapper that composes only when its member happens to draw is worse
    #: than one that never does, since it survives casual testing.
